...
def _get_hit_miss_counts_numba(prediction, truth, thresholds):
    ...
    """
    thresholds = np.asarray(thresholds, dtype=prediction.dtype)
    prediction = prediction[:, :, np.newaxis, :, :]
    truth = truth[:, :, np.newaxis, :, :]
    thresholds = thresholds[np.newaxis, np.newaxis, :, np.newaxis, np.newaxis]

    pred_hit = prediction >= thresholds
    truth_hit = truth >= thresholds

    TP = np.logical_and(pred_hit, truth_hit).sum(axis=(3, 4)).transpose(1, 0, 2)
    FN = np.logical_and(~pred_hit, truth_hit).sum(axis=(3, 4)).transpose(1, 0, 2)
    FP = np.logical_and(pred_hit, ~truth_hit).sum(axis=(3, 4)).transpose(1, 0, 2)
    TN = np.logical_and(~pred_hit, ~truth_hit).sum(axis=(3, 4)).transpose(1, 0, 2)
...
class Evaluater:
    ...
    def calculate_stat(self):
        ...
        n = TP + FP + FN + TN
        aref = (TP + FP) * (TP + FN) / (n + epsilon)
        gss = (TP - aref) / (TP + FP + FN - aref + epsilon)
        hss = 2 * (TP * TN - FP * FN) / ((TP + FN) * (FN + TN) + (TP + FP) * (FP + TN) + epsilon)
        mse = self.mse / max(1, self.total_batch_num)
        mae = self.mae / max(1, self.total_batch_num)
    ...
    def save_txt_readable(self, path):
        ...
        for (i, threshold) in enumerate(self.threshold):
            ...
            f.write(f"   Recall: {list(recall[:, i])}\n")
            f.write(f"   F1: {list(f1[:, i])}\n")
            ...
            f.write(f"   Recall stat: avg {recall[:, i].mean()}/final {recall[-1, i]}\n")
            f.write(f"   F1 stat: avg {f1[:, i].mean()}/final {f1[-1, i]}\n")
