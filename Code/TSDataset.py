...
def list_files(src_dir, extension):
    return [os.path.join(src_dir, f) for f in os.listdir(src_dir) if f.endswith(extension)]
...
class TSDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        sample_dirs = [
            dir_name for dir_name in sorted(os.listdir(data_dir))
            if os.path.isdir(os.path.join(data_dir, dir_name))
        ]
        self.input_dirs = [os.path.join(data_dir, dir_name, 'input') for dir_name in sample_dirs]
        self.label_dirs = [os.path.join(data_dir, dir_name, 'label') for dir_name in sample_dirs]
        self.transform = transform

        # Process metadata
        metadata = Path(data_dir).name.split("_")
