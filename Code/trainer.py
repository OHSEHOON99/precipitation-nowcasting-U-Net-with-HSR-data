...
def setup_wandb(
    ...
    if not enabled:
        return None
    if wandb is None:
        raise RuntimeError("wandb is not installed. Install it or run without --wandb.")
    ...
    experiment = wandb.init(
        ...
        mode=mode,
        resume='allow',
        anonymous='allow' if api_key is None else None,
    )
...
def train_model(
        ...
        wandb_project: str = 'precipitation-nowcasting',
    ...
    early_stopping = EarlyStopping(patience=early_stopping_patience, save_path=save_dir / f'{run_name}_best_model.pth')
    ...
    for epoch in range(N_epochs):
        ...
        with tqdm(total=total_batches, desc=f'Epoch {epoch + 1}/{N_epochs}', unit='batch') as pbar:
            for batch_idx, batch in enumerate(train_loader):
                ...
                if experiment is not None and global_step % 25 == 0:
                    experiment.log({'train_loss': loss.item(), 'global_step': global_step})

        # Validation loop
        val_loss, histograms, visualizations = validate_model(
            model,
            val_loader,
            criterion,
            device,
            amp_enabled,
            evaluater,
            log_to_wandb=experiment is not None,
        )
        ...
        if visualizations is not None:
            log_data['visualization'] = visualizations
        log_data.update(histograms)
        ...
        logging.info(f'Epoch {epoch + 1}: train_loss={avg_train_loss:.4f}, val_loss={avg_val_loss:.4f}')

        if save_checkpoint:
            torch.save(model.state_dict(), save_dir / f'{run_name}_epoch{epoch + 1}.pth')

        # Check early stopping condition
        if early_stopping.should_stop(model, avg_val_loss):
    ...
    model = early_stopping.load(model, device=device)

    # Save the final model
    torch.save(model.state_dict(), save_dir / f'{run_name}_final_model.pth')
...
def validate_model(model, val_loader, criterion, device, amp_enabled, evaluater, n_classes=1, n_channels=6, log_to_wandb=False):
    """
    ...
    with torch.no_grad():  # Disable gradient computation for validation
        ...
        for tag, value in model.named_parameters():
            tag = tag.replace('/', '.')
            if log_to_wandb and wandb is not None and not (torch.isinf(value) | torch.isnan(value)).any():
                histograms['Weights/' + tag] = wandb.Histogram(value.data.cpu())
            if log_to_wandb and wandb is not None and value.grad is not None and not (torch.isinf(value.grad) | torch.isnan(value.grad)).any():
